...
export default function SearchBox() {
	...
	const [hits, setHits] = useState([]);
		...
		async function onSearch(e) {
			...
			try {
				let data = await searchHooks(q);
				let hooks = data?.hooks || [];
				// remove the hit that’s identical to the just-generated hook
				if (genRes?.hook) {
					const genTxt = genRes.hook.trim();
					hooks = hooks.filter(h => (h.text || "").trim() !== genTxt);
				}
				// remove duplicates by text/topic/style
				const seen = new Set();
				hooks = hooks.filter(h => {
					const key = `${(h.text||"").trim()}|${(h.topic||"").trim()}|${(h.style||"").trim()}`;
